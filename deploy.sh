...
cf push ${PROTECTED_APP_NAME} --var app-name="${PROTECTED_APP_NAME}" --path app --manifest app/manifest.yml
...
cf push "${ROUTE_SERVICE_APP_NAME}" --no-start --var proxy-app-name="${ROUTE_SERVICE_APP_NAME}" --path proxy --manifest proxy/manifest.yml
